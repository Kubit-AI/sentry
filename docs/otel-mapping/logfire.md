...
| `ai.response.msToFirstChunk` (also legacy `ai.stream.msToFirstChunk` for AI SDK <4.0) | `gen_ai.response.time_to_first_chunk` | Vercel emits TTFT for streaming calls in **milliseconds** (int) on `ai.streamText.doStream` / `ai.streamObject.doStream` spans. The OTel-spec equivalent is in **seconds** (float) — kubit-otel reads ms-typed aliases first, falling back to the seconds alias × 1000. Canonical Kubit field: `time_to_first_token` (ms, int). |
