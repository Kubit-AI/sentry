...
- Tests in `nodejs-sdk/tests/transformer.normalization.test.ts` and `python-sdk/tests/test_normalization.py` are kept structurally identical (43 cases each, same fixtures). Python adds one extra case (`test_synthesis_recovers_python_repr_args_via_literal_eval`) covering the PY-only `repr(dict)` recovery in the Langfuse tool-span synthesizer; that asymmetry is intentional and documented in `docs/otel-mapping/langfuse.md`.
